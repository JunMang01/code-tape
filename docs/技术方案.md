...
| 状态管理 | React hooks + Context + reducer；复杂共享状态再引入 Zustand | P0 先按状态作用域拆清责任，避免过早引入全局复杂 store |
| 样式实现 | TailwindCSS + CSS variables + 少量全局 CSS（启动前最终确认） | 提高工具台 UI 开发效率，主题由 token 承载 |
...
P0 不以“是否全局 store”为第一问题，而以状态的作用域、更新频率和可测试性拆分：

- **hooks**：承接组件局部 UI 状态映射，例如 tab、输入框、loading、hover/open、面板展开、按钮 disabled 等。
- **Context**：承接低频、跨局部组件树共享的上下文，例如主题 token、当前录制包 id、只读配置、服务依赖注入；不承接高频事件流和播放器逐帧状态。
- **reducer**：承接局部流程型 UI 状态，用显式 action 表达状态迁移，例如录制控制条、回放控制条、保存流程、权限申请流程。
- **框架无关核心模块**：`RecordingClock`、`EventBus`、`RecordingController`、`ReplayScheduler`、`ReplayReducer`、schema validators 保持纯 TypeScript 或低 React 耦合；React 只负责订阅状态和派发命令。
- **Zustand 引入触发条件**：当录制页、回放页、媒体面板、事件面板等跨模块共享状态导致 Context 嵌套过深、props drilling 明显、多个面板频繁订阅同一状态时，再引入小型 Zustand store。
- **暂不采用 Redux Toolkit / XState 作为 P0 默认**：Redux 对 Demo 规模偏重；XState 可在 P1 状态机复杂度显著上升时重新评估。
...
- 避免装饰性大 hero 和营销文案。
- P0 必须先建立 CSS variables / design tokens；默认深色用于 Demo。主题能力与主题切换入口分开：token 化是 P0 基础能力，深浅主题 toggle 是 P0 可选增强，不阻塞录制、回放、保存主链路。
- P0 token 至少覆盖：`background`、`foreground`、`border`、`surface`、`muted`、`primary`、`danger`、`warning`、`success`、`focus ring`、`tooltip`、`popover`。
- 若团队接受原子类，P0 推荐 TailwindCSS + CSS variables + 少量全局 CSS：Tailwind 负责布局、间距、hover、focus、disabled 状态；CSS variables 负责主题语义；Radix primitives 负责可访问交互；lucide-react 负责图标。
- 若启动前确认不采用 TailwindCSS，则使用 CSS Modules 或普通 CSS 文件，但必须保留同一套 design tokens，不把颜色和间距散落在组件实现里。
